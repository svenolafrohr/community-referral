...
describe('ViewToggle', () => {
  it('marks the active mode as pressed', () => {
    ...
    expect(screen.getByRole('button', { name: 'Show as cards' })).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByRole('button', { name: 'Show as list' })).toHaveAttribute('aria-pressed', 'false')
    expect(screen.getByRole('button', { name: 'Show as stack' })).toHaveAttribute('aria-pressed', 'false')
  })

  it('calls onChange with the selected mode', () => {
    const onChange = vi.fn()
    render(<ViewToggle value="grid" onChange={onChange} />)
    fireEvent.click(screen.getByRole('button', { name: 'Show as stack' }))
